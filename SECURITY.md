...
- The Action runs entirely on the workflow's runner. It executes the repository's own `npm ci` and declared scripts inside a disposable clone of the pull request's **base** commit, so a PR cannot make the Action run code that the base branch does not already contain. Reviewers still own the trust decision for scripts in their own repository, as with any CI job.
- Inputs are `package`, `version`, `attempt-repair`, `comment`, model settings, tokens, `report-dir`, and `node-version`. There is no input for commands, scripts, executables, or environment variables; `package` must be a valid npm name and `version` an exact semantic version. Detection from a pull request only reads `package.json` at the base and head commits.
