...
import type { DependencySection, PackageManifest } from '../core/types.js';
...
export type DependencyDetection =
  { ok: true; change: DetectedDependencyChange } | { ok: false; reason: string; changes: string[] };
...
export function detectDependencyChange(base: PackageManifest, head: PackageManifest): DependencyDetection {
  ...
  const labels = changes.map(
    (change) =>
      `${change.section}.${change.packageName}: ${change.fromRange ?? '(absent)'} → ${change.toRange ?? '(removed)'}`,
  );
  const upgrades = changes.filter(
    (change): change is ManifestChange & { fromRange: string; toRange: string } =>
      change.fromRange !== null && change.toRange !== null,
  );
  if (!changes.length)
    return {
      ok: false,
      reason: 'The pull request does not change any dependency range in package.json.',
      changes: labels,
    };
  if (upgrades.length !== 1 || changes.length !== 1) {
    return {
      ok: false,
      reason:
        'The pull request changes more than one dependency, or adds/removes one. Pass `package` and `version` inputs to investigate a single upgrade.',
      changes: labels,
    };
  }
  ...
  if (!targetVersion) {
    return {
      ok: false,
      reason: `The new range "${change.toRange}" for ${change.packageName} does not resolve to an exact version.`,
      changes: labels,
    };
  }
  return {
    ...
    change: {
      ...
      fromRange: change.fromRange,
      toRange: change.toRange,
      targetVersion,
    },
  };
}
