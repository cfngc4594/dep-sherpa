...
import type { RepairInvestigationContext } from '../core/types';
import {
...
const context: RepairInvestigationContext = {
  finding: { packageName: 'zod', section: 'dependencies', declaredRange: '^3.23.8', currentVersion: '3.23.8', targetVersion: '4.1.5', releaseType: 'major', risk: 'high', reasons: [] },
  diagnostics: ["src/validation.ts(4,16): error TS2339: Property 'errors' does not exist on type 'ZodError<unknown>'."],
  sources: [{ path: 'src/validation.ts', startLine: 1, endLine: 5, content: 'return error.errors.map(String);', diagnostic: 'd' }],
...
const proposalPayload = {
  ...
  proposal: {
    ...
    edits: [{ path: 'src/validation.ts', expectedText: 'return error.errors.map(String);', replacement: 'return error.issues.map(String);', rationale: 'v4 renamed errors to issues', diagnostic: 'd' }],
  },
};
...
function fakeFetch(responder: (request: { url: string; body: Record<string, unknown>; headers: Headers }, call: number) => Response) {
  let calls = 0;
...
describe('model configuration', () => {
  it('uses any OpenAI-compatible endpoint from OPENAI_API_KEY and OPENAI_BASE_URL', () => {
    expect(resolveModelConfig({ OPENAI_API_KEY: 'sk-test' })).toEqual({ provider: 'openai-compatible', baseURL: undefined, apiKey: 'sk-test', model: defaultOpenAIModel });
    expect(resolveModelConfig({ OPENAI_API_KEY: 'sk-test', OPENAI_BASE_URL: 'https://llm.example/v1', DEPSHERPA_MODEL: 'qwen-coder' }))
      .toEqual({ provider: 'openai-compatible', baseURL: 'https://llm.example/v1', apiKey: 'sk-test', model: 'qwen-coder' });
  });

  it('accepts a key-less self-hosted endpoint', () => {
    expect(resolveModelConfig({ OPENAI_BASE_URL: 'http://localhost:11434/v1', DEPSHERPA_MODEL: 'llama3' })).toMatchObject({ provider: 'openai-compatible', apiKey: 'no-key-required', model: 'llama3' });
  });
  ...
  it('never turns the GitHub Actions token into a model credential (GitHub Models is retired)', () => {
    ...
    expect(resolveModelConfig({ GITHUB_ACTIONS: 'true', GITHUB_TOKEN: 'ghs_token', OPENAI_API_KEY: 'sk-test' })).toMatchObject({ provider: 'openai-compatible', apiKey: 'sk-test' });
  });
  ...
  it('fails closed with an actionable reason when nothing is configured', () => {
    ...
    expect(describeModelConfig(resolveModelConfig({ OPENAI_API_KEY: 'k', OPENAI_BASE_URL: compatibleBaseUrl, DEPSHERPA_MODEL: 'qwen' }))).toBe('qwen @ llm.example');
    expect(describeModelConfig(resolveModelConfig({ OPENAI_API_KEY: 'k' }))).toBe(`${defaultOpenAIModel} @ api.openai.com`);
  });
});
...
describe('structured output schema', () => {
  it('is a strict-mode object schema without provider-hostile constraints', () => {
    ...
    expect(stripSchemaConstraints({ a: { minLength: 1, type: 'string' }, b: [{ maxItems: 2 }] })).toEqual({ a: { type: 'string' }, b: [{}] });
  });
});
...
describe('OpenAI-compatible proposal generator', () => {
  it('sends the packet with a strict JSON schema and returns a validated agent proposal', async () => {
    ...
    const generate = createOpenAIProposalGenerator({ provider: 'openai-compatible', baseURL: compatibleBaseUrl, apiKey: 'sk-test', model: 'gpt-4.1-mini' }, { fetch: impl });
    ...
    expect(calls[0].body.response_format).toMatchObject({ type: 'json_schema', json_schema: { name: 'repair_generation', strict: true } });
  ...
  it('falls back to json_object when a provider rejects json_schema', async () => {
    const { impl, calls } = fakeFetch((_request, call) => (
      call === 1
        ? new Response(JSON.stringify({ error: { message: 'response_format json_schema is not supported' } }), { status: 400, headers: { 'content-type': 'application/json' } })
        : completion(JSON.stringify({ outcome: 'no_proposal', reason: 'Insufficient evidence.', proposal: null }))
    ));
    const generate = createOpenAIProposalGenerator({ provider: 'openai-compatible', baseURL: 'https://llm.example/v1', apiKey: 'k', model: 'local' }, { fetch: impl });
  ...
  it('never fabricates a proposal from invalid, empty, or non-conforming output', async () => {
    ...
    await expect(invalidJson(context)).resolves.toMatchObject({ status: 'unavailable', reason: expect.stringContaining('valid JSON') });
    const empty = createOpenAIProposalGenerator(config, { fetch: fakeFetch(() => completion(null)).impl });
    await expect(empty(context)).resolves.toMatchObject({ status: 'unavailable', reason: expect.stringContaining('no content') });
    const wrongShape = createOpenAIProposalGenerator(config, { fetch: fakeFetch(() => completion(JSON.stringify({ outcome: 'proposal', proposal: { id: 'x' } }))).impl });
    await expect(wrongShape(context)).resolves.toMatchObject({ status: 'unavailable', reason: expect.stringContaining('machine-valid') });
    const missingProposal = createOpenAIProposalGenerator(config, { fetch: fakeFetch(() => completion(JSON.stringify({ outcome: 'proposal', reason: null, proposal: null }))).impl });
  ...
  it('reports endpoint failures as unavailable instead of throwing', async () => {
    const { impl } = fakeFetch(() => new Response(JSON.stringify({ error: { message: 'bad credentials' } }), { status: 401, headers: { 'content-type': 'application/json' } }));
    const generate = createOpenAIProposalGenerator({ provider: 'openai-compatible', baseURL: compatibleBaseUrl, apiKey: 'expired', model: 'gpt-4.1-mini' }, { fetch: impl });
    const result = await generate(context);
  ...
  it('is unavailable without any configuration and performs no request', async () => {
    ...
    const generate = createOpenAIProposalGenerator({ provider: 'none', reason: 'No model is configured.' }, { fetch: impl });
