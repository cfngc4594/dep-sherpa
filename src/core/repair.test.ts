...
describe('bounded source repair', () => {
  ...
  it('plans the documented TanStack Query v5 cacheTime option migration on the exact failing line', async () => {
    ...
    const diagnostic = {
      ...
      output: "src/query.ts(4,71): error TS2769: Object literal may only specify known properties, and 'cacheTime' does not exist in type 'UseQueryOptions'.",
    };
...
async function trackedRepository(files: Record<string, string>, typecheckScript = 'node -e "process.exit(0)"'): Promise<string> {
  const root = await mkdtemp(path.join(tmpdir(), 'depsherpa-proposal-test-'));
  temporaryRoots.push(root);
  const allFiles = {
    'package.json': JSON.stringify({ name: 'proposal-fixture', private: true, scripts: { typecheck: typecheckScript } }, null, 2),
    ...files,
  };
...
function proposal(pathName = 'src/editor.ts', expectedText = 'export const mode = legacyMode();', replacement = 'export const mode = modernMode();'): RepairProposal {
  return {
    kind: 'agent',
...
describe('generic repair proposal policy', () => {
  ...
  it('rejects untracked files', async () => {
    ...
    const result = await validateRepairProposal(root, proposal('src/untracked.ts'), context([sourceContext('src/untracked.ts')]));
  ...
  it('rejects stale or ambiguous exact context', async () => {
    ...
    const stale = await validateRepairProposal(root, proposal(), context([sourceContext('src/editor.ts', 'export const mode = anotherMode();')]));
    expect(stale).toMatchObject({ ok: false });

    const ambiguousProposal = proposal('src/editor.ts', 'legacyMode()', 'modernMode()');
    const ambiguous = await validateRepairProposal(root, ambiguousProposal, context([sourceContext('src/editor.ts', 'export const mode = legacyMode();\nexport const second = legacyMode();')]));
  ...
  it('rejects more than three files and more than twelve changed lines', async () => {
    const files = Object.fromEntries([1, 2, 3, 4].map((number) => [`src/file${number}.ts`, `export const value${number} = legacyMode();`]));
    const root = await trackedRepository(files);
    const sources = Object.entries(files).map(([filePath, content]) => ({ path: filePath, startLine: 1, endLine: 1, content, diagnostic: genericDiagnostic(filePath) }));
    const tooManyFiles: RepairProposal = {
      ...proposal(),
      edits: sources.map((source) => ({ path: source.path, expectedText: source.content, replacement: source.content.replace('legacyMode', 'modernMode'), rationale: 'compiler evidence', diagnostic: source.diagnostic })),
    };
  ...
  it.each(['tests/editor.ts', 'src/fixtures/editor.ts', 'migrations/editor.ts', 'src/config/editor.ts', 'vite.config.ts'])(
    'rejects tests, fixtures, migrations, and configuration: %s',
    async (blockedPath) => {
  ...
  it('rejects a tracked but unrelated file that was not in the diagnostic context', async () => {
    ...
    const result = await validateRepairProposal(root, proposal('src/unrelated.ts'), context([sourceContext('src/editor.ts')]));
    expect(result).toMatchObject({ ok: false });
  ...
  it('rejects binary content even when the extension is allowed', async () => {
    ...
    const result = await validateRepairProposal(root, binaryProposal, context([sourceContext('src/editor.ts', 'legacy')]));
    expect(result).toMatchObject({ ok: false });
  ...
  it('rejects type-check suppression and process execution in replacements', async () => {
    ...
    for (const replacement of ['// @ts-ignore\nexport const mode = legacyMode();', 'process.exit(0);']) {
      const result = await validateRepairProposal(root, proposal('src/editor.ts', 'export const mode = legacyMode();', replacement), repairContext);
      expect(result).toMatchObject({ ok: false });
...
describe('generic proposal verification loop', () => {
  function failedGenericTypecheck(): CommandResult {
    return { name: 'typecheck', command: 'npm run typecheck', status: 'failed', exitCode: 2, durationMs: 1, output: genericDiagnostic() };
  }
  ...
  it('validates, applies, and verifies a non-recipe Agent proposal', async () => {
    ...
    const result = await attemptBoundedRepair({
      ...
      releaseEvidence: ['Installed package manifest: installed=monaco-editor@0.51.0.'],
      proposalGenerator: async (repairContext) => ({ status: 'generated', proposal: proposal(repairContext.sources[0].path, repairContext.sources[0].content) }),
    });
  ...
  it('reports failed verification without claiming success', async () => {
    const root = await trackedRepository({ 'src/editor.ts': 'export const mode = legacyMode();' }, 'node -e "process.exit(1)"');
    const result = await attemptBoundedRepair({
      requested: true,
      workspacePath: root,
      finding: genericFinding,
  ...
  it('degrades clearly when the model provider is unavailable', async () => {
    ...
    const result = await attemptBoundedRepair({
      ...
      checks: [{ name: 'typecheck', command: 'npm run typecheck', available: true }],
      candidateResults: [failedGenericTypecheck()],
      proposalGenerator: async () => ({ status: 'unavailable', reason: 'AWS credentials or model access are unavailable.' }),
    });
