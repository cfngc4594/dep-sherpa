...
import { renderIsolatedUpgradeReport, renderMarkdownReport } from './report';
...
describe('report rendering', () => {
  ...
  it('renders isolation, comparison, and patch evidence without implying an external write', () => {
    const command = { name: 'test', command: 'npm run test', status: 'passed' as const, exitCode: 0, durationMs: 5, output: 'ok' };
    const report: IsolatedUpgradeReport = {
      mode: 'isolated-local', generatedAt: '2026-09-02T00:00:00.000Z', repository: 'sample', manifestPath: '/tmp/workspace/package.json', packageManager: 'npm',
      finding: { packageName: 'zod', section: 'dependencies', declaredRange: '4.5.3', currentVersion: '4.5.3', targetVersion: '4.5.4', releaseType: 'patch', risk: 'low', reasons: ['Patch change.'] },
      checks: [{ name: 'test', command: 'npm run test', available: true }], results: [command], externalWritesAllowed: false,
