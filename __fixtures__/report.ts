import type { CommandResult, IsolatedUpgradeReport, RepairAttempt } from '../src/core/types.js';
...
export function sampleRepairAttempt(overrides: Partial<RepairAttempt> = {}): RepairAttempt {
  return {
    ...
    releaseEvidence: [
      'Requested exact target: zod@4.1.5.',
      'Installed package manifest: installed=zod@4.1.5; homepage=https://zod.dev.',
    ],
...
export function sampleIsolatedUpgradeReport(overrides: Partial<IsolatedUpgradeReport> = {}): IsolatedUpgradeReport {
  ...
  return {
    ...
    checks: ['lint', 'typecheck', 'test', 'build'].map((name) => ({
      name,
      command: `npm run ${name}`,
      available: true,
    })),
    ...
    source: {
      path: '/Users/sample/projects/checkout',
      gitHead: 'cdd613954d4f9b2a32d6667b441d9456ef73f46f',
      dirtyFilesIgnored: [],
    },
    ...
    upgrade: {
      name: 'apply_upgrade',
      command: 'npm install --ignore-scripts --no-audit --no-fund --save-exact zod@4.1.5',
      status: 'passed',
      exitCode: 0,
      durationMs: 900,
      output: '',
    ...
    candidateResults: [
      result('lint', 'passed'),
      result('typecheck', 'failed', typecheckFailure),
      result('test', 'passed'),
      result('build', 'failed', typecheckFailure),
    ],
    ...
    repairSuggestions: [
      {
        ...
        summary: 'The upgrade introduced a typecheck failure.',
        nextAction: 'Start with the first compiler diagnostic.',
        evidence: typecheckFailure,
      },
      {
        check: 'build',
        classification: 'introduced_failure',
        summary: 'The upgrade introduced a build failure.',
        nextAction: 'Trace the earliest build error.',
        evidence: typecheckFailure,
      },
    ],
    repair,
