...
`action.yml` declares a Node 24 JavaScript action whose entry point is the committed Rollup bundle `dist/index.js` (the `actions/typescript-action` layout; `check-dist.yml` fails CI when the bundle is stale). `src/index.ts` calls `run()` in `src/main.ts`, which reads inputs with `@actions/core`, derives the pull-request context from `@actions/github`, and hands both to `src/action/run.ts` together with the toolkit sinks (job summary, outputs, annotations) and an Octokit-backed comment publisher. The orchestration is toolkit-agnostic: `inputs.ts` validates the typed inputs, `detect.ts` derives the single dependency change from the base and head manifests, `source.ts` fetches the base commit by SHA when the checkout is shallow and clones it into a temporary directory through a short-lived local branch, the core's `upgradeInIsolation` runs against that clone, `comment.ts` renders the compact PR comment, and `github.ts` creates or updates the single marked comment. `@actions/artifact` uploads the report files. Verdicts never fail the job; only invalid inputs or infrastructure errors do.
