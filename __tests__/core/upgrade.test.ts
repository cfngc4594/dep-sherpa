...
describe('isolated upgrade planning', () => {
  ...
  it('blocks on setup failures and otherwise prioritizes introduced regressions', () => {
    ...
    expect(
      classifyVerdict(passed, passed, [
        { name: 'test', baseline: 'passed', candidate: 'failed', state: 'introduced_failure' },
      ]),
    ).toBe('needs_repair');
    expect(
      classifyVerdict(passed, passed, [
        { name: 'test', baseline: 'failed', candidate: 'failed', state: 'pre_existing_failure' },
      ]),
    ).toBe('inconclusive');
    expect(
      classifyVerdict(passed, passed, [{ name: 'test', baseline: 'passed', candidate: 'passed', state: 'passed' }]),
  ...
  it('retains the first actionable diagnostic and bounds noisy output', () => {
    ...
    expect(diagnosticExcerpt(output)).toBe(
      'src/index.ts(8,3): error TS2339: Property issues does not exist · context line · more context',
    );
